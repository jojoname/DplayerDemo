...
class Danmaku {
    ...
    load() {
        ...
        if (this.options.api.maximum) {
            apiurl = `${this.options.api.address}v3/?vid=${this.options.api.id}&max=${this.options.api.maximum}`;
        } else {
            apiurl = `${this.options.api.address}v3/?vid=${this.options.api.id}`;
        }
    ...
    check(value) {
        if (typeof this.options.check == 'function') {
            const text = this.options.check(value, { vid: this.options.id, author: this.options.api.user });
            return typeof (text || {}).then === 'function' ? text : Promise.resolve(text);
        }
    ...
    draw(dan) {
        if (this.showing) {
            ...
            for (let i = 0; i < dan.length; i++) {
                ...
                if (dan[i].style) {
                    let style = '';
                    Object.keys(dan[i].style).forEach((k) => {
                        style += `${k}:${dan[i].style[k]};`;
                    });
                    item.innerHTML = `<span style="${style}">${dan[i].text}</span>`;
                } else {
                ...
                item.style.fontSize = `${dan[i].font || 20}px`;
                item.addEventListener('animationend', () => {
                    this.container.removeChild(item);
                });
                item.addEventListener('mouseover', (e) => {
                    this.events &&
                        this.events.trigger('danmaku_item_mouseover', {
                            event: e,
                            data: dan[i],
                        });
                });
                item.addEventListener('mousemove', (e) => {
                    this.events &&
                        this.events.trigger('danmaku_item_mousemove', {
                            event: e,
                            data: dan[i],
                        });
                });
                item.addEventListener('mouseout', (e) => {
                    this.events &&
                        this.events.trigger('danmaku_item_mouseout', {
                            event: e,
                            data: dan[i],
                        });
                });
                item.addEventListener('click', (e) => {
                    this.events &&
                        this.events.trigger('danmaku_item_click', {
                            event: e,
                            data: dan[i],
                        });
                });
                ...
                switch (dan[i].type) {
                    ...
                    case 'bottom':
                        tunnel = getTunnel(item, dan[i].type);
                        if (tunnel >= 0) {
                            item.style.bottom = `${this.options.cHeight + itemHeight * tunnel}px`;
                        }
                        break;
                    default:
    ...
    play(id) {
        if (id) {
            const el = this.container.querySelector(`div[data-id="${id}"]`);
            if (el) {
                el.classList.remove('dplayer-danmaku-item-paused');
                return;
            }
        }
    ...
    pause(id) {
        if (id) {
            const el = this.container.querySelector(`div[data-id="${id}"]`);
            if (el) {
                el.classList.add('dplayer-danmaku-item-paused');
                return;
            }
        }
        this.paused = true;
