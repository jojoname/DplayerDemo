...
class Setting {
    constructor(player) {
        ...
        if (this.player.danmaku) {
            ...
            this.player.bar.set('danmakuFont', 0.5, 'width');
            this.player.template.fontInput.value = utils.font[`0.5`];

            const danmakuFontMove = (event) => {
                const e = event || window.event;
                let percentage = ((e.clientX || e.changedTouches[0].clientX) - utils.getBoundingClientRectViewLeft(this.player.template.danmakuFontBarWrap)) / 184;
                if (percentage <= 0.125) {
                    percentage = 0;
                ...
                this.player.bar.set('danmakuFont', percentage, 'width');
                this.player.template.fontInput.value = utils.font[`${percentage}`];
            };
            const danmakuFontUp = () => {
                document.removeEventListener(utils.nameMap.dragEnd, danmakuFontUp);
                document.removeEventListener(utils.nameMap.dragMove, danmakuFontMove);
            };

            this.player.template.danmakuFontBarWrapWrap.addEventListener('click', (event) => {
                const e = event || window.event;
                let percentage = ((e.clientX || e.changedTouches[0].clientX) - utils.getBoundingClientRectViewLeft(this.player.template.danmakuFontBarWrap)) / 184;
                if (percentage <= 0.125) {
                    percentage = 0;
            ...
            this.player.template.danmakuFontBarWrapWrap.addEventListener(utils.nameMap.dragStart, () => {
                document.addEventListener(utils.nameMap.dragMove, danmakuFontMove);
                document.addEventListener(utils.nameMap.dragEnd, danmakuFontUp);
            });
        }
    }
