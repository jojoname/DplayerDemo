...
class Comment {
    ...
    send() {
        ...
        const text = this.player.template.commentInput.value;
        // text can't be empty
        if (!text.replace(/^\s+|\s+$/g, '')) {
            this.player.notice(this.player.tran('please-input-danmaku'));
            return;
        }
        this.player.danmaku.check(text).then(({ text, error, sendSuccess }) => {
            if (error) {
                this.player.notice(text);
                return;
            }
            this.player.danmaku.send(
                {
                    text: text,
                    font: parseInt(this.player.template.fontInput.value),
                ...
                () => {
                    ...
                    this.hide();
                    typeof sendSuccess === 'function' && sendSuccess(text);
                }
            );
        });
    }
